...
int ICACHE_FLASH_ATTR cgiWiFiConnect(HttpdConnData *connData) {
	...
	// First ensure that all variables are nulled out
	for (i = 0; i < sizeof(sysCfg.mqtt_pass); i++)
		sysCfg.mqtt_pass[i] = 0;

	for (i = 0; i < sizeof(sysCfg.mqtt_user); i++)
		sysCfg.mqtt_user[i] = 0;

	for (i = 0; i < sizeof(sysCfg.mqtt_host); i++)
		sysCfg.mqtt_host[i] = 0;

	for (i = 0; i < sizeof(sysCfg.base); i++)
		sysCfg.base[i] = 0;
	...
	sysCfg.mqtt_port = atoi(mqtt_port); // Convert mqtt_port string to integer
	...
	if (!strcmp(s, "yes"))
		sysCfg.enable_webpage_control = 1;
	else
		sysCfg.enable_webpage_control = 0;




	CFG_Save();
	INFO("\r\nSSID=%s, WiFi Pwd=%s,Host=%s,Port= %d,User=%s,PW=%s, Device=%s",  \
				sysCfg.sta_ssid, sysCfg.sta_pwd, sysCfg.mqtt_host, sysCfg.mqtt_port, sysCfg.mqtt_user, sysCfg.mqtt_pass, sysCfg.base);
...
if (os_strcmp(token, "WiFiMode")==0)
	...
	if (os_strcmp(token, "web_pass")==0)
		{
		os_strcpy(buff, (char*) sysCfg.web_pass);
		}
else
	if (os_strcmp(token, "currSsid")==0)
		{
	...
	if (os_strcmp(token, "web_enabled_yes") == 0)
		{
		if  (sysCfg.enable_webpage_control == 1)
			os_strcpy(buff, " checked");
		}
if (os_strcmp(token, "web_enabled_no") == 0)
	{
	if  (sysCfg.enable_webpage_control == 0)
		os_strcpy(buff, " checked");
	}
else
	if (os_strcmp(token, "WiFiPasswd")==0)
		{
